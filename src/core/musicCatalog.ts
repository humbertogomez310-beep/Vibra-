...
export interface LibraryTrack extends Song {
  ...
  lastPlayedAt?: number;
  favorite?: boolean;
  /** Huella (nombre+tamaño+fecha) usada para detectar duplicados al escanear. */
  fingerprint?: string;
  /** Indica si esta pista tiene un archivo de audio real guardado en IndexedDB. */
  hasAudio?: boolean;
  /** Indica si esta pista tiene una portada real guardada en IndexedDB. */
  hasCover?: boolean;
}
...
export class LocalMusicCatalog {
  ...
  hydrate(tracks: LibraryTrack[] = []): LibraryTrack[] {
    this.tracks = tracks.map((track) => this.normalizeTrack(track));
    this.rebuildIndex();
  ...
  /**
   * Carga la biblioteca persistida en IndexedDB (metadatos + audio + portada)
   * y la fusiona en el catálogo en memoria. Se llama una vez al iniciar la
   * aplicación (ver MoodPanel). Resuelve URLs de blob reproducibles para
   * cada pista con audio/portada real, sin tocar PlayerEngine ni AutoDJ:
   * ambos ya leen `song.audioUrl` / `song.cover` tal cual.
   */
  async hydrateFromStorage(): Promise<LibraryTrack[]> {
    if (!libraryDb.isAvailable()) {
      return this.getTracks();
    }

    try {
      const storedMeta = await memoryEngine.getLibraryTracks();

      const resolvedTracks = await Promise.all(
        storedMeta.map(async (meta) => {
          const track = this.normalizeTrack(meta);

          if (meta.hasAudio) {
            const audioBlob = await libraryDb.getTrackAudioBlob(meta.id);
            if (audioBlob) {
          ...
          if (meta.hasCover) {
            const coverBlob = await libraryDb.getTrackCoverBlob(meta.id);
            if (coverBlob) {
              track.cover = URL.createObjectURL(coverBlob);
            }
          }
      ...
      console.error('No se pudo cargar la biblioteca desde IndexedDB:', error);
    }
  ...
  removeTrack(trackId: string): LibraryTrack | null {
    ...
    this.releaseTrackUrls(removedTrack);
    this.rebuildIndex();
    this.persist();
    void libraryDb.deleteTrack(trackId);
    eventBus.emit(LIBRARY_TRACK_EVENTS.REMOVED, removedTrack);
    return removedTrack;
  }

  /** Revoca las blob URLs de audio/portada para evitar fugas de memoria. */
  private releaseTrackUrls(track: LibraryTrack): void {
